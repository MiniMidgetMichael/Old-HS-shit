...
class TurtleScreen(TurtleScreenBase):
    ...
    def onclick(self, fun, btn=1, add=None, args=None):
        """Bind fun to mouse-click event on canvas.
        ...
        >>> screen.onclick(None)
        """
        self._onscreenclick(fun, btn, add, args)
...
##############################  stamp stuff  ###############################
    ...
    def write(self, arg, move=False, align="left", font=("Arial", 8, "normal")):
        ...
        if self.undobuffer:
            self.undobuffer.push(["seq"])
            self.undobuffer.cumulate = True
        end = self._write(str(arg), align.casefold(), font)
        if move:
            x, y = self.pos()
            self.setpos(end, y)
        if self.undobuffer:
...
def read_docstrings(lang):
    ...
    """
    modname = "turtle_docstringdict_%(language)s" % {'language':lang.casefold()}
